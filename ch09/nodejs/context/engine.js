...
/**
 * ContextEngine — 上下文管理引擎（第九章扩展版）
 *
 * 相较于第六/八章，本章新增：
 *   - skillsContent：技能列表内容，由 SkillManager.formatForPrompt() 提供
 *   - buildSystemPrompt 新增 {skills} 占位符替换
 */
export class ContextEngine {
   ...
   * @param {string} [skillsContent]  - 技能列表内容（第九章新增，注入 {skills} 占位符）
   */
  constructor(policies = [], memory = null, contextWindow = 200_000, skillsContent = '') {
    this.systemPromptTemplate = '';
    this.messages = /** @type {MessageWrap[]} */ ([]);
    this.policies = policies;
    this.memory = memory;
    this.skillsContent = skillsContent; // 第九章新增
  ...
  /**
   * 构建 system prompt（替换占位符：{runtime} / {workspace_path} / {memory} / {skills}）
   *
   * {skills}：注入技能列表（第九章新增）
   */
  buildSystemPrompt() {
    ...
    const skillsContent  = this.skillsContent || 'No skills available.';

    return this.systemPromptTemplate
      .replace('{runtime}', runtime)
      .replace('{workspace_path}', workspacePath)
      .replace('{memory}', memoryContent)
      .replace('{skills}', skillsContent); // 第九章新增
  }
