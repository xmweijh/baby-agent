...
export class BabyAgentTUI {
  ...
  #handleEvent(event) {
    switch (event.type) {
      ...
      case MessageTypeIndex:
        this.#endSections();
        if (event.index.running) {
          process.stdout.write(col(C.blue, '[RAG] 索引代码库中...'));
        } else if (event.index.err) {
          console.log(col(C.red, ` 索引失败: ${event.index.err.message}`));
        } else {
          console.log(col(C.blue, ` 索引完成${event.index.summary ? ': ' + event.index.summary : ''}`));
        }
        break;
      default: break;
    }
  ...
  #renderWelcome() {
    console.log(col(C.cyan, 'BabyAgent TUI — ch07 (Agentic RAG)'));
    console.log(col(C.gray, '────────────────────────────────────────────────'));
